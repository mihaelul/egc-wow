#version 330

// Input
in vec3 world_position;
in vec3 world_normal;

// Uniforms for light properties
uniform vec3 light_direction; 
uniform vec3 light_position;
uniform vec3 eye_position;

//second light
uniform vec3 light2_direction;
uniform vec3 light2_position;

uniform float material_kd;
uniform float material_ks;
uniform int material_shininess;

//second light
uniform float material2_kd;
uniform float material2_ks;
uniform int material2_shininess;


// TODO(student): Declare any other uniforms
uniform int is_spotlight;

uniform vec3 object_color;

// Output
layout(location = 0) out vec4 out_color;


void main()
{

     vec3 N = normalize(world_normal);
     vec3 L = normalize( light_position - world_position );
     vec3 V = normalize( eye_position - world_position );
     vec3 H = normalize( L + V );
     vec3 R = reflect (-L, N);

     vec3 L2 = normalize(light2_position - world_position);

    // TODO(student): Define ambient, diffuse and specular light components
    float ambient_light = 0.25;

    //Compute the diffuse light component
    float diffuse_light = material_kd * max(dot(normalize(N), L), 0);
    float specular_light = 0;

    // It's important to distinguish between "reflection model" and
    // "shading method". In this shader, we are experimenting with the Phong
    // (1975) and Blinn-Phong (1977) reflection models, and we are using the
    // Phong (1975) shading method. Don't mix them up!
    if (diffuse_light > 0)
    {
        specular_light = material_ks * pow(max(dot(normalize(N), H), 0), material_shininess);
    }

    // Second light source
    float diffuse_light2 = material2_kd * max(dot(normalize(N), normalize(light2_position - world_position)), 0);

	// Compute specular light component for second light
	float specular_light2 = 0;

    if (diffuse_light2 > 0)
	{
		vec3 H2 = normalize(L2 + V);
		specular_light2 = material2_ks * pow(max(dot(normalize(N), H2), 0), material2_shininess);
	}


    // TODO(student): If (and only if) the light is a spotlight, we need to do
    // some additional things.

    // Compute light
	float light = 0;
    if(is_spotlight == 1)
    {
        // TODO(student): Compute spotlight effect
        float cut_off = radians(30.0f);
        float spot_light = dot(-L, light_direction);
        float spot_light_limit = cos(cut_off);
        if (spot_light > spot_light_limit)
		    {
			    float linear_att = (spot_light - spot_light_limit) / (1.0f - spot_light_limit);
			    float light_att_factor = pow(linear_att, 2);
			    light = ambient_light + light_att_factor * (diffuse_light + specular_light);
		    } 
		else
		    {
			    light = ambient_light; 
		    }
	} 
	else
	{
		light = ambient_light + diffuse_light + specular_light;
	}

        //compute total light ambient + attenuation +(diffuse+specular) + attenuation2 +(diffuse2+specular2);
            light = light + diffuse_light2 + specular_light2;
   

    // TODO(student): Compute the total light. You can just add the components
    // together, but if you're feeling extra fancy, you can add individual
    // colors to the light components. To do that, pick some vec3 colors that
    // you like, and multiply them with the respective light components.


    // TODO(student): Write pixel out color
    vec3 colour = object_color * light;
    out_color = vec4(colour,1);

}
