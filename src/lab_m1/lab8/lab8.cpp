...
void Lab8::Init()
{
    ...
    // Light & material properties
    {
       ...
       lightPosition2 = glm::vec3(2, 1, 1);
        lightDirection2 = glm::vec3(0, -1, -1);
        material2Shininess = 50;
        material2Kd = 0.5;
        material2Ks = 0.5;
    }
}
...
void Lab8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    int material2_kd = glGetUniformLocation(shader->program, "material2_kd");
    glUniform1f(material2_kd, material2Kd);

    int material2_ks = glGetUniformLocation(shader->program, "material2_ks");
    glUniform1f(material2_ks, material2Ks);

    int material2_shininess = glGetUniformLocation(shader->program, "material2_shininess");
    glUniform1i(material2_shininess, material2Shininess);
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        ///second light
		// Control light position using on Y, H, J, K, U, I
        if (window->KeyHold(GLFW_KEY_U)) lightPosition2 -= forward * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_H)) lightPosition2 -= right * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_J)) lightPosition2 += forward * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_K)) lightPosition2 += right * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_Y)) lightPosition2 += up * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_I)) lightPosition2 -= up * deltaTime * speed;
