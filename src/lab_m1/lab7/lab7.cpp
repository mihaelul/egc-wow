...
{
    {
        ...
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, glm::vec3(1.f, 0, 0));
    }
    ...
    // Render ground
    {
        ...
        RenderSimpleMesh(meshes["plane"], shaders["LabShader"], modelMatrix, glm::vec3(0, 0, 1.f));
    }
...
void Lab7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    const GLint loc_lightPosition = glGetUniformLocation(shader->program, "light_position");
    glUniform3fv(loc_lightPosition, 1, value_ptr(lightPosition));

    glm::vec3 eyePosition = GetSceneCamera()->m_transform->GetWorldPosition();
    // Set eye position (camera position) uniform
    const GLint loc_eyePosition = glGetUniformLocation(shader->program, "eye_position");
    glUniform3fv(loc_eyePosition, 1, value_ptr(eyePosition));

    // Set material property uniforms (shininess, kd, ks, object color)
    const GLint loc_materialShininess = glGetUniformLocation(shader->program, "material_shininess");
    glUniform1i(loc_materialShininess, materialShininess);

    const GLint loc_materialKd = glGetUniformLocation(shader->program, "material_kd");
    glUniform1f(loc_materialKd, materialKd);

    const GLint loc_materialKs = glGetUniformLocation(shader->program, "material_ks");
    glUniform1f(loc_materialKs, materialKs);

    const GLint loc_object_color = glGetUniformLocation(shader->program, "object_color");
