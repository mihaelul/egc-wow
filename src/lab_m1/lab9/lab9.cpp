...
void Lab9::Update(float deltaTimeSeconds)
{
...

    {
        ...
        model_matrix = translate(model_matrix, glm::vec3(3.0f, 1.5f, 1.0f));
        model_matrix = scale(model_matrix, glm::vec3(0.5f));

        mixTextures = true;
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], model_matrix, mapTextures["grass"], mapTextures["earth"]);
        mixTextures = false;
    }
}
